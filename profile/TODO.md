...
### High
- **Game scores not persisted** — Live game state exists in memory/Redis but final scores are never written to Postgres when a game ends. The `answers` table is designed for this but the write never happens, so post-game review and historical results don't work.
- **No tests** — Zero Go test files, no Jest/Vitest config, no testing libraries in package.json. No coverage on backend, frontend, or WebSocket flow.
