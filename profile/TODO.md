...
### GKE deployment blockers (audit 2026-04-25)

The Helm chart at `charts/charts/quibble/` (separate `charts` repo) is functional but built for ingress-nginx + cert-manager, not GKE-native. Stripe is intentionally deferred. Top items to address before a GKE deploy:

- [ ] **Auth0 missing on the web tier** — `templates/web-deployment.yaml` only sets `API_URL`. The Next.js Auth0 SDK requires `AUTH0_SECRET`, `AUTH0_CLIENT_ID`, `AUTH0_CLIENT_SECRET`, `AUTH0_BASE_URL`, `AUTH0_ISSUER_BASE_URL`. Add `web.env` and `web.secret` blocks to `values.yaml`, thread the secret values into `templates/secret.yaml`, and reference them from the web Deployment. Without this, the frontend cannot authenticate. (Pairs with the existing Critical item "Auth0 frontend not wired up".)
- [ ] **No goose migrations runner in the chart** — there is no `Job`, init container, or Helm hook to run migrations against Neon before the API starts. A cold install will fail. Note: API does support `AUTO_MIGRATE=true` (per the resolved deployment infra item), so the simpler fix is to set that env var in `templates/api-deployment.yaml` and document it; otherwise add a `pre-install,pre-upgrade` hook Job that runs goose with the same `DATABASE_URL`.
- [ ] **GHCR `imagePullSecrets` not configured** — `api.image` and `web.image` default to `ghcr.io/benbotsford/trivia-{api,web}` (private). Neither Deployment references an `imagePullSecrets`. Add `imagePullSecrets` plumbing to `values.yaml` and both Deployment templates, plus docs for creating a `docker-registry` secret from a GHCR PAT. Longer-term: mirror images to Artifact Registry and use Workload Identity to drop the pull secret entirely.
- [ ] **No `sessionAffinity: ClientIP` on the API Service** — `templates/api-service.yaml` is a vanilla ClusterIP. The realtime Hub in `apps/api/internal/realtime/hub.go` is in-process; scaling beyond `replicaCount: 1` will break WebSocket games as clients round-robin across pods. Add `sessionAffinity: ClientIP` (and `sessionAffinityConfig.clientIP.timeoutSeconds`) as a guardrail. If using GKE-native Ingress (`gce`), also add a `BackendConfig` with `connectionDraining` and a long `timeoutSec` for the WS upgrade.
- [ ] **Database connection pool unbounded against Neon** — `apps/api/cmd/server/main.go` uses pgx defaults (~100 conns/pod). Neon's lower tiers cap concurrent connections in the low double digits. Add `MaxConns`, `MinConns`, `MaxConnLifetime` to `internal/config/config.go` (default `MaxConns=10`), pass them through `pgxpool.ParseConfig`, and use Neon's `-pooler` host in `DATABASE_URL` for the app while keeping the direct host for migrations.

GKE-vs-DOKS ingress decision (values-level, not a chart rewrite): either install `ingress-nginx` + `cert-manager` add-ons on the GKE cluster and ship the chart as-is, or override `ingress.className: gce`, replace the `cert-manager.io/cluster-issuer` annotation with a `ManagedCertificate` resource, and add the `BackendConfig` mentioned above.
